...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        // Setup window and root viewController
        
        window = UIWindow(frame: UIScreen.main.bounds)
        window?.makeKeyAndVisible()
        
        let teamViewController = TeamViewController(collectionViewLayout:UICollectionViewFlowLayout())
        let navigationController = UINavigationController(rootViewController:teamViewController)
        window?.rootViewController = navigationController
        
        UINavigationBar.appearance().barTintColor = UIColor.white
       
        // Remove text from back button display 
